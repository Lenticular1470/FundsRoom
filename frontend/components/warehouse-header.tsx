...
export default function WarehouseHeader({
  ...
  const { user } = useAuth()
  const currentUser = user || { name: 'bilibibili', email: 'bi@hji.com' }
      ...
      <div className="flex items-center gap-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-64 pl-9 pr-4 py-1.5 bg-[#0c121e] border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-[#0092b8] transition-colors"
          />
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 bg-[#0c121e] border border-slate-800 rounded-xl text-xs">
          <div className="w-5 h-5 rounded-full bg-amber-400/20 text-amber-400 border border-amber-400/30 flex items-center justify-center font-bold text-[10px]">
            {currentUser.name ? currentUser.name.charAt(0).toLowerCase() : 'b'}
          </div>
          <span className="font-bold text-slate-200">{currentUser.name || 'bilibibili'}</span>
        </div>
      </div>
    </div>
  )
